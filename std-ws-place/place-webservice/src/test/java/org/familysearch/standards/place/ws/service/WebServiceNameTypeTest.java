...
public class WebServiceNameTypeTest extends WebServiceBaseTest {

    /** Note: this will actually be a 'spy' on the real service */
    private WebServiceNameType webService;
    ...
    public void beforeMethod() throws URISyntaxException {
        ...
        WebServiceNameType realWebService = new WebServiceNameType();
    ...
    //
    // ------------------------------------------------------------------------------------------------
    // Get [Read] place-name-type by id methods
    // ------------------------------------------------------------------------------------------------
    //
    @Test(groups = {"unit"} )
    public void getPlaceNameTypeById() throws PlaceDataException {
        doReturn(this.createDTO(this.mockPlaceNameType())).when(readableServiceMock).getTypeById(eq(TypeCategory.NAME), anyInt());
    ...
    //
    // ------------------------------------------------------------------------------------------------
    // Get [Read] all published place-name-types
    // ------------------------------------------------------------------------------------------------
    //
    @Test(groups = {"unit"} )
    public void getPlaceNameTypes() throws PlaceDataException {
        // Create three published place-name-types and one unpublished one
        Set<TypeDTO> nameTypes = new HashSet<TypeDTO>();
        nameTypes.add(this.createDTO(this.mockPlaceNameType()));
        nameTypes.add(this.createDTO(this.mockPlaceNameType()));
        nameTypes.add(this.createDTO(this.mockPlaceNameType()));
        PlaceNameType nameType = this.mockPlaceNameType();
        when(nameType.isPublished()).thenReturn(false);
        nameTypes.add(this.createDTO(nameType));

        doReturn(nameTypes).when(readableServiceMock).getAllTypes(TypeCategory.NAME);
    ...
    public void createNameType() throws PlaceDataException {
        ...
        PlaceNameType type = this.mockPlaceNameType();
        TypeModel typeModel = new TypeMapper(readableServiceMock).createModelFromType(type, StdLocale.ENGLISH, "/test", TypeCategory.NAME);
        inRoot.setType(typeModel);

        doReturn(this.createDTO(type)).when(writableServiceMock).create(eq(TypeCategory.NAME), isA(TypeDTO.class), anyString());
        ...
        assertEquals(String.valueOf(response.getMetadata().get("Location").get(0)), "/test/places/name-types/" + type.getId());
    }

    //
    // ------------------------------------------------------------------------------------------------
    // PUT [update] Name-type
    // ------------------------------------------------------------------------------------------------
    //
    @Test(groups = {"unit"} )
    public void updateNameType() throws PlaceDataException {
        ...
        doReturn(this.createDTO(type)).when(writableServiceMock).update(eq(TypeCategory.NAME), isA(TypeDTO.class), anyString());
