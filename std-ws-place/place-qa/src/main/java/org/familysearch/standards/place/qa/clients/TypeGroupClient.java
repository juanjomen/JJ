 ...
 */
public class TypeGroupClient extends MasterClient {

  private URI ourResourceEndpoint;

  public TypeGroupClient() {
    super();
    ourResourceEndpoint = UriBuilder.fromUri(PLACE_URI).path("/places/type-groups").build();
  }
  ...
  public ClientResponse post(PlaceModel theTypeGroup) {
    UriBuilder builder = UriBuilder.fromUri(ourResourceEndpoint);
    WebResource web = client.resource(builder.build());
    MultivaluedMap<String, String> parameters = new MultivaluedMapImpl();
    return web.queryParams(parameters).entity(theTypeGroup).post(ClientResponse.class);
  }

  public ClientResponse put(PlaceModel theTypeGroup) {
    UriBuilder builder = UriBuilder.fromUri(ourResourceEndpoint).path(theTypeGroup.getId().toString());
    WebResource web = client.resource(builder.build());
    MultivaluedMap<String, String> parameters = new MultivaluedMapImpl();
    return web.queryParams(parameters).entity(theTypeGroup).put(ClientResponse.class);
  }
