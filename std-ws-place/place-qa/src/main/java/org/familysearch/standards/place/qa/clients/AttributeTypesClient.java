 ...
 */
public class AttributeTypesClient extends MasterClient {

  private URI ourResourceEndpoint;

  public AttributeTypesClient() {
    super();
    ourResourceEndpoint = UriBuilder.fromUri(PLACE_URI).path("/places/attribute-types").build();
  }
  ...
  public ClientResponse post(AttributeModel theAttribute) {
    UriBuilder builder = UriBuilder.fromUri(ourResourceEndpoint);
    WebResource web = client.resource(builder.build());
    MultivaluedMap<String, String> parameters = new MultivaluedMapImpl();
    return web.queryParams(parameters).entity(theAttribute).post(ClientResponse.class);
  }

  public ClientResponse put(AttributeModel theAttribute) {
    UriBuilder builder = UriBuilder.fromUri(ourResourceEndpoint).path(theAttribute.getId().toString());
    WebResource web = client.resource(builder.build());
    MultivaluedMap<String, String> parameters = new MultivaluedMapImpl();
    return web.queryParams(parameters).entity(theAttribute).put(ClientResponse.class);
  }
