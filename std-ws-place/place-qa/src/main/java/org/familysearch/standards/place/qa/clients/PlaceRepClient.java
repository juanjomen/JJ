 ...
 */
public class PlaceRepClient extends MasterClient {

  private URI ourResourceEndpoint;

  public PlaceRepClient() {
    super();
    ourResourceEndpoint = UriBuilder.fromUri(PLACE_URI).path("/places/reps").build();
  }
  ...
  public ClientResponse post(PlaceModel thePlaceRep) {
    UriBuilder builder = UriBuilder.fromUri(ourResourceEndpoint);
    WebResource web = client.resource(builder.build());
    MultivaluedMap<String, String> parameters = new MultivaluedMapImpl();
  ...
  public ClientResponse put(PlaceModel thePlaceRep) {
    UriBuilder builder = UriBuilder.fromUri(ourResourceEndpoint).path(thePlaceRep.getId().toString());
    WebResource web = client.resource(builder.build());
    MultivaluedMap<String, String> parameters = new MultivaluedMapImpl();
